"use client";
import { Button } from "@/components/ui/button";
import {
  Dialog,
  DialogClose,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Textarea } from "@/components/ui/textarea";
import { Plus } from "lucide-react";
import { useForm } from "react-hook-form";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { toast } from "sonner";
import { createNote, CreateNoteInput } from "@/app/utils/notesAPI";
import { useTranslations } from "next-intl";
import { z } from "zod";
import { zodResolver } from "@hookform/resolvers/zod";
import { AxiosError } from "axios";

const addNoteSchema = z.object({
  title: z.string().min(1, "Title is required").max(200),
  content: z.string().min(1, "Content is required").max(5000),
  tag: z.string().max(50),
});

type AddNoteFormData = z.infer<typeof addNoteSchema>;

export function AddNote() {
  const t = useTranslations("modals.notes");
  const tc = useTranslations("modals.common");
  const tn = useTranslations("dashboard.notes");
  const queryClient = useQueryClient();

  const {
    register,
    handleSubmit,
    reset,
    formState: { errors },
  } = useForm<AddNoteFormData>({
    resolver: zodResolver(addNoteSchema),
    defaultValues: { title: "", content: "", tag: "" },
  });

  const mutation = useMutation({
    mutationFn: (data: CreateNoteInput) => createNote(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["notes"] });
      toast.success(tn("toast.createSuccess"));
      reset();
    },
    onError: (error: AxiosError<{ message?: string }>) => {
      toast.error(tn("toast.createError"), {
        description: error.response?.data?.message,
      });
    },
  });

  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className="flex items-center add-btn" aria-label={tn("addNote")}>
          <Plus className="w-4 h-4" aria-hidden="true" />
          {tn("addNote")}
        </button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[450px] w-full">
        <form
          onSubmit={handleSubmit((data) => mutation.mutate(data))}
          className="space-y-4"
        >
          <DialogHeader>
            <DialogTitle>{t("create.title")}</DialogTitle>
            <DialogDescription>{tc("fillDetails")}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 mt-2">
            <div className="grid gap-2">
              <Label htmlFor="note-title">{t("create.titleLabel")}</Label>
              <Input
                id="note-title"
                {...register("title")}
                placeholder={t("create.titlePlaceholder")}
                aria-invalid={!!errors.title}
              />
              {errors.title && (
                <p className="text-sm text-red-500" role="alert">{errors.title.message}</p>
              )}
            </div>
            <div className="grid gap-2">
              <Label htmlFor="note-content">{t("create.contentLabel")}</Label>
              <Textarea
                id="note-content"
                {...register("content")}
                placeholder={t("create.contentPlaceholder")}
              />
              {errors.content && (
                <p className="text-sm text-red-500" role="alert">{errors.content.message}</p>
              )}
            </div>
            <div className="grid gap-2">
              <Label htmlFor="note-tag">{t("create.tagLabel")}</Label>
              <Input
                id="note-tag"
                {...register("tag")}
                placeholder={t("create.tagPlaceholder")}
              />
            </div>
          </div>
          <DialogFooter className="mt-4 flex justify-end gap-2">
            <DialogClose asChild>
              <Button variant="outline">{tc("cancel")}</Button>
            </DialogClose>
            <Button
              type="submit"
              className="bg-green-600 hover:bg-green-700"
              disabled={mutation.isPending}
            >
              {t("create.submit")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
}
